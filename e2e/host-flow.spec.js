...
import { HOST_PASSWORD, HOST_USERNAME, joinQueue, loginAsHost, rowFor, SLUG } from './helpers'
...
test.describe('acciones sobre una fila', () => {
  test('"Quitar" pide confirmación y cierra el ticket del comensal', async ({ page, browser }) => {
  ...
  test('"No vino" solo aplica a quien ya fue llamado, y pide confirmación', async ({ page, browser }) => {
    const host = await browser.newContext()
    const hostPage = await host.newPage()
    await loginAsHost(hostPage)
  ...
  test('repetir "Llamar" no duplica el efecto', async ({ page, browser }) => {
    const host = await browser.newContext()
    const hostPage = await host.newPage()
    await loginAsHost(hostPage)
