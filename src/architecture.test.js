...
const MODULE_LAYERS = ['domain', 'schemas', 'services', 'hooks', 'components', 'pages']
...
const SHARED = ['shared-utils', 'shared-constants']
...
const AREA_RULES = {
  ...
  services_global: ['services_global', 'shared-utils'],
}
...
export function classify(rel) {
  ...
  if (parts[0] === 'modules' && parts[1]) {
    ...
    if (parts.length === 2 || (parts.length === 3 && /^index(\.jsx?)?$/.test(parts[2]))) {
      return { kind: 'module-index', module: parts[1] }
    }
    if (MODULE_LAYERS.includes(parts[2])) return { kind: 'module', module: parts[1], layer: parts[2] }
...
export function violationsFor(fileRel, imports) {
  const source = classify(fileRel)
  if (source.kind === 'composition') return []
  const found = []
  const sourceKey = source.kind === 'module' ? source.layer : source.kind
  ...
  for (const { spec, targetRel } of imports) {
    ...
    if (source.kind === 'module') {
      const rules = MODULE_RULES[source.layer]
      if (target.kind === 'module' && target.module === source.module) {
        ...
        else if (!rules.same.includes(target.layer)) found.push(`${source.layer} no debe importar ${target.layer} (${spec})`)
...
describe('arquitectura por capas', () => {
  ...
  it.each(files.map(({ rel }) => rel))('%s respeta sus dependencias', (rel) => {
    const { file } = files.find((f) => f.rel === rel)
    expect(violationsFor(rel, importsOf(file))).toEqual([])
  })
})

describe('las reglas detectan las violaciones', () => {
  const imp = (spec, targetRel = spec.startsWith('@/') ? spec.slice(2) : null) => ({ spec, targetRel })
