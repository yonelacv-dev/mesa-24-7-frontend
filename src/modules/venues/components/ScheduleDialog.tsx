...
import type { DayDraft, ScheduleBody } from '@/shared/types'
...
interface ScheduleDialogProps {
  ...
  /** Borrador inicial: [{ is_open, start, end }] de lunes a domingo. */
  days: DayDraft[]
...
function ScheduleEditor({ initialDays, timezoneLabel, pending, errorMessage, onCancel, onSave }: ScheduleEditorProps) {
  const [draft, setDraft] = useState(initialDays)
  const errors = scheduleErrors(draft)
  const invalid = Object.keys(errors).length > 0
  const setDay = (index: number, patch: Partial<DayDraft>) => setDraft((current) => current.map((d, i) => (i === index ? { ...d, ...patch } : d)))
