import { makeListStatus } from '@/test/factories'
...
describe('closedNotice', () => {
  it('no hay aviso si la lista está abierta', () => {
    expect(closedNotice(makeListStatus({ kind: 'open', closes_at: '01:00' }))).toBeNull()
  })

  it('en pausa explica que pregunte al anfitrión', () => {
    expect(closedNotice(makeListStatus({ kind: 'paused' }))).toEqual({
  ...
  it('cerrada dice cuándo abre', () => {
    const notice = closedNotice(makeListStatus({ kind: 'closed', next_open: { day_offset: 0, weekday: 4, time: '11:00' } }))
...
describe('listStatusBadge', () => {
  it('describe cada estado', () => {
    expect(listStatusBadge(makeListStatus({ kind: 'open', closes_at: '01:00' }))).toEqual({
      label: 'Abierta hasta las 01:00',
      tone: 'open',
    })
    expect(listStatusBadge(makeListStatus({ kind: 'paused' })).label).toBe('En pausa')
    expect(listStatusBadge(makeListStatus({ kind: 'closed' })).label).toBe('Cerrada')
  })
})

describe('hostClosedText', () => {
  const closed = makeListStatus({ kind: 'closed', next_open: { day_offset: 1, weekday: 5, time: '11:00' } })

  it('no dice nada si no está cerrada', () => {
    expect(hostClosedText(makeListStatus({ kind: 'open' }), 3)).toBeNull()
  })
