...
const called = makeRow({
  status: 'called',
  called_at: '2026-09-19T01:00:00Z',
  hold_ends_at: '2026-09-19T01:10:00Z',
})
...
describe('chips de una fila', () => {
  it('esperando muestra un solo chip neutro', () => {
    expect(rowChips(makeRow({ status: 'waiting' }), 10, 0, at('01:00:00'))).toEqual([
...
describe('isOverdue', () => {
  it('solo un llamado con el plazo vencido', () => {
    ...
    expect(isOverdue(makeRow({ status: 'waiting' }), 0, at('01:11:00'))).toBe(false)
  })
})
