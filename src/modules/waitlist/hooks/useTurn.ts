...
import type { DinerStreamMessage, Entry } from '@/shared/types'
...
export function useTurn(slug: string, token: string) {
  ...
  const connection = useEventStream<DinerStreamMessage>({
    path: entryStreamPath(slug, token),
    enabled: !notFound,
    onMessage: (message) => {
      if (message.event === 'state') queryClient.setQueryData(key, message.data)
    },
  })
  ...
  const applyEntry = (entry: Entry) => queryClient.setQueryData(key, entry)
