 ...
 */
export function useHostActions(slug: string) {
  const queryClient = useQueryClient()
  const key = queryKeys.hostQueue(slug)

  const mutation = useMutation({
    mutationFn: ({ entryId, action }: { entryId: number; action: HostAction }) => runHostAction(slug, entryId, action),
    onSuccess: (queue) => queryClient.setQueryData(key, queue),
    onError: () => queryClient.invalidateQueries({ queryKey: key }),
  })

  return {
    run: (entryId: number, action: HostAction) => mutation.mutate({ entryId, action }),
    pendingEntryId: mutation.isPending ? (mutation.variables?.entryId ?? null) : null,
